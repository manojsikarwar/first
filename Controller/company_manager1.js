...
module.exports.company_manager_profile = (user) => {
    return new Promise((resolve, reject) => {
        ...
        const id = user.id;
        if (role_id == 3) {
            const sql = `select * from registration where company_id = '${company_id}' 
		   				 and id = '${id}'`;
		...
		if(role_id1 == 3 || role_id1 == 2){
			if(first_name != '' && email != '' && password != '' && role_id != '' 
				...
				bcrypt.hash(password,10,function(err,hash){
					if(role_id == 4){
						const user = `select email from registration where
						company_id = '${company_id}' and super_id = '${id}' and email = '${email}' `
...
module.exports.manager_user_list = (user) => {
	...
	const role_id 		= user.role_id;
 	...
 	const id 			= user.id;
	   if(role_id == 3){
		   	const sql = `select * from registration where company_id = '${company_id}'
		   				and super_id = '${id}' and role_id = '${4}' and status = '${0}'`;
...
module.exports.company_manager_survey = (user,body) =>{
	return new Promise((resolve,reject)=>{
		...
		if(id != ''){
			if(role_id == 3 || role_id == 2){
				const sql = `select res_id,title,desc1,start_date,final_date from
				 research where id = '${id}' `
...
module.exports.company_manager_dashboard = (user) => {
	return new Promise((resolve,reject)=>{
		...
		if(role_id == 3){
				...
				company_id = '${company_id}' and super_id = '${id}' and role_id = '${4}'
				 and status = '${0}'`;
				client.query(user,(err,usercount)=>{ 			
					...
					resolve(CountData)
						...
						where id = '${id}' and final_date <= '${myDate}' `
