...
module.exports.costumer_admin_reg = (body) => {
					...
					if(role_id == 2 || role_id == 3){
						...
						client.query(sql,(err,ress)=>{
							if(ress.rows != ''){
								...
								const sql1 = `insert into registration(first_name,email,password,role_id,role_type,super_id,create_by,last_name,confirm_password,company_name)
								values(
...
//Post
module.exports.admin_change_password = (user,body) =>{
	return new Promise((resolve,reject)=>{
	  	const id  		  = user.id;
...
module.exports.admin_view_profle = (id) => {
	return new Promise((resolve,reject)=>{
...
//=================== manager_list ================================
//get
module.exports.manager_list = (role_id) => {
	return new Promise((resolve,reject)=>{
		if(role_id == 2){
			const sql1 = `select * from registration where role_id = '${3}'`
			client.query(sql1,(err,ress1)=>{
				if(err){
					const Data = {
						"success" : false,
...
//=========================  admin_update_profle  ============================
//Put
module.exports.admin_update_profle = (user,body) => {
	return new Promise((resolve,reject)=>{
		const id 		= user.id;
	    const role_id 	= user.role_id;
	    ...
	    if(role_id == 2){
	    	...
	    	const Data = {
	    		"success" : false,
	    		"message":"This is Not a Admin Profile So you Can't Update"
	    	}
	    	resolve(Data)
	    }
	})
...
module.exports.admin_delete_manager = (id,role_id) => {
	return new Promise((resolve,reject)=>{
		if(id != ''){
		 	if(role_id == 2){
		 		const sql1 = `select * from registration where id ='${id}'`;
