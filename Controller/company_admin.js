...
module.exports.company_create_module = (user,body) => {
	return new Promise((resolve,reject)=>{
		...
		const status 			= 0;
		if(id == 2){
			if(first_name != '' && email != '' && password != '' && role_type != '' 
				&& super_id != '' && create_by != '' && last_name != '' 
				&& company_id != ''){
				if(role_type == 'manager' || role_type == 'admin' || role_type == 'user' ){
				const sql = `select * from registration where company_id = '${company_id}' and 
				email = '${email}'`;
				client.query(sql,(err,ress)=>{
					if(ress.rows != ''){
						...
						if(role_type == 'manager'){
							...
							password,role_id,role_type,super_id,create_by,last_name,
							confirm_password,company_name,company_id,status)
							values(
								'${first_name}','${email}','${hash}','${3}','${role_type}',
								'${super_id}','${create_by}','${last_name}','${hash}',
						...
						if(role_type == 'admin'){
							...
							confirm_password,company_name,company_id,status)
							values(
							'${first_name}','${email}','${hash}','${2}','${role_type}',
							'${super_id}','${create_by}','${last_name}','${hash}',
							'${company_name}','${company_id}','${status}')`;
							client.query(sql1,(err1,ress1)=>{
					 			if(err){
						...
						if(role_type == 'user'){
							...
							confirm_password,company_name,company_id,status)
							values(
							'${first_name}','${email}','${hash}','${4}','${role_type}',
							'${super_id}','${create_by}','${last_name}','${hash}',
							'${company_name}','${company_id}','${status}')`;
							client.query(sql1,(err1,ress1)=>{
					 			if(err){
...
module.exports.company_manager_list = (user,company_id) => {
	return new Promise((resolve,reject)=>{
		...
		if(role_id == 2 || role_id == 1 || role_id == 3){
			if(company_id != ''){
		 		const sql1 = `select id,first_name,last_name,email,role_type,company_id
		 		 from registration where company_id = '${company_id}' and role_id = '${3}'
		 		 and status = '${0}' `
		 		client.query(sql1,(err,ress)=>{
		 			if(ress.rows == ''){
		 				const Data = {
...
module.exports.company_admin_list = (user,company_id) => {
	return new Promise((resolve,reject)=>{
		...
		if(role_id == 2 || role_id == 1){
			if(company_id != ''){
		 		const sql1 = `select c.status,c.company_name,r.id,r.first_name,
		 		r.last_name,r.email,r.role_type from create_company as c 
		 		inner join registration as r on c.company_id = r.company_id 
...
module.exports.company_user_list = (user,company_id) => {
	return new Promise((resolve,reject)=>{
		...
		if(role_id == 2 || role_id == 1 || role_id == 3){
			if(company_id != ''){
		 		const sql1 = `select company_id from registration where
		 		company_id = '${company_id}' and status = '${0}' `;
		 		client.query(sql1,(err,ress)=>{
		 			// resolve(ress.rows)
		 			if(ress.rows == ''){
		 				const Data = {
			 			...
			 			if(err){
							...
							resolve(Data)
			 				...
			 				const user = `select * from registration where
			 				company_id = '${id}' and role_id = '${4}' and status = '${0}' `
...
module.exports.company_delete = (role_id,id) => {
	return new Promise((resolve,reject)=>{
		if(role_id == 1){
			if(id != ''){
		 		const sql1 = `select * from create_company where 
		 		company_id = '${id}'`;
		 		client.query(sql1,(err,ress)=>{
		 			if(ress.rows == ''){
						const Data = {
						...
						resolve(Data)
		 				...
		 				const sql1 = `update create_company set status = '${1}'
		 				...
		 				client.query(sql1,(err,ress1)=>{
		 					if(err){
		 						...
		 						const sql1 = `update registration set status = '${1}'
		 						 where company_id = '${ress.rows[0].company_id}' `
				 				client.query(sql1,(err,ress1)=>{
				 					if(err){
				 						const Data = {
...
module.exports.company_edit = (user,body) => {
	return new Promise((resolve,reject)=>{
		...
		if(id != '' && company_name != '' && company_address != ''
		 	&& company_phone != '' && comp_email != ''){
			if(role_id == 2){
				if(id != ''){
			 		...
			 		client.query(sql1,(err,ress)=>{
			 			if(ress.rows == ''){
							...
							resolve(Data)
			 				...
			 				const sql1 = `update create_company set 
			 					company_name = '${company_name}',company_address = '${company_address}',
			 					company_phone = '${company_phone}',comp_email = '${comp_email}' 
...
module.exports.dhashbord = (role_id) => {
	return new Promise((resolve,reject)=>{
		if(role_id == 1){
	 		const sql1 = `select count(status) from create_company where
	 		 status = '${0}'`;
	 		client.query(sql1,(err,companycount)=>{ 			
	 			if(err){
					...
					const sql1 = `select count(start_date) from research where
					 final_date <= '${myDate}' `
					client.query(sql1,(err,surveycount)=>{
						...
						const sql2 = `select count(final_date) from research
						 where final_date >= '${myDate}' `
...
module.exports.company_admin_delete_module = (body,user) => {
	return new Promise((resolve,reject)=>{
		...
		if(id != ''){
		 	if(role_id == 2){
		 		const sql1 = `select * from registration where
		 		...
		 		client.query(sql1,(err,admindel)=>{
					if(admindel.rows == ''){
						...
						if(role == 3 || role == 4){
					 		 const sql = `update registration set status = '${1}'
					 		  where id = '${id1}'`;
...
module.exports.company_admin_profile = (user) => {
	...
	const role_id 		= user.role_id;
 	...
 	const id 			= user.id;
	    if(role_id == 2){
		   	const sql = `select * from registration where 
...
module.exports.company_admin_dashboard = (user) => {
	return new Promise((resolve,reject)=>{
		...
		const company_id = user.company_id;
		if(role_id == 2){
	 		const manager = `select count(status) from registration where 
	 		company_id = '${company_id}' and role_id = '${3}' and status = '${0}'`;
	 		client.query(manager,(err,managercount)=>{ 			
	 			if(err){
					...
					resolve(CountData)
			 		...
			 		const user = `select count(status) from registration where 
			 		company_id = '${company_id}' and role_id = '${4}' and status = '${0}'`;
			 		client.query(user,(err,usercount)=>{ 			
			 			if(err){
							...
							const compeleteSurvey = `select count(start_date) from research 
							where company_id = '${company_id}' and final_date <= '${myDate}' `
							client.query(compeleteSurvey,(err,surveycount)=>{
								...
								const runningSurvey = `select count(final_date) from research 
								where company_id = '${company_id}' and final_date >= '${myDate}' `
...
module.exports.company_profile = (user) => {
	...
	const role_id 		= user.role_id;
 	const company_id 	= user.company_id;
	    if(role_id == 2){
		   	const sql = `select * from create_company where 
		   	company_id = '${company_id}' and status = '${0}' `;
			client.query(sql,(err,ress)=>{
				if(err){
					const Data = {
...
module.exports.company_add_user = (body,user) => {
		...
		if(role_id1 == 3 || role_id1 == 2){
			if(first_name != '' && email != '' && role_id != '' && role_type != '' 
				&& create_by != '' && last_name != '' && confirm_password != ''){
				var password = generator.generate({
					...
					bcrypt.hash(password,10,function(err,hash){
						if(role_id == 4){
							...
							'${id}','${create_by}','${last_name}','${confirm_password}',
							'${company_name}','${company_id}','${status}'
							)`;
							client.query(sql1,(err,ress1)=>{
								if(err){
...
module.exports.company_add_user = (body,user) => {
	...
	const status 			= 0;
		if(role_id1 == 2 || role_id1 == 3){
			const user1 = `select * from registration where email = '${email}' and
						   super_id = '${id}'`
