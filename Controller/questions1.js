...
module.exports.insert = (body,user) => {
	return new Promise(async(resolve,reject)=>{
		...
		if(role_id == 2 || role_id == 3){
			const sql = `insert into research(title,desc1,question_count,
				 invited_users_count,start_date,final_date,created_at,
				 survey_submited,id,company_id) 
				 values(
				'${title}','${desc1}','${question_count}','${invited_users_count}',
				'${start_date}','${end_date}','${created_at}','${survey_submited}',
				'${id}','${company_id}') RETURNING res_id`;
			client.query(sql,(err,ress)=>{
				if(err){
					...
					for(let invited_user of body.users){
						...
						const sql6 = `insert into invited_user(user_id, survey_id,
						 invited_at, is_submit, submit_at)
						 values('${user_id}','${res_id}','${invited_at}','${is_submit}',
						 '${submit_at}') RETURNING id`
						client.query(sql6,(err,ress6)=>{
							if(err){
								...
								const userName = `select first_name,last_name from registration
								where id = '${user_id}' `
								client.query(userName,(err,userNameress)=>{
									if(err){
										...
										for(let key1 of userNameress.rows){
											...
											let status	= 'false';
											const status1 = `insert into survey_status(invited_id,user_id,
											created_at,updated_at,status,survey_id,first_name,last_name)
											values('${invited_id}','${user_id}','${myDate}','${myDate}','${status}',
											'${res_id}','${first_name}','${last_name}') `
					...
					for(let item1 of body.questions){
						...
						const sql1 =`insert into questions(research_id,question_type,question,options_count,redirection_type,created_at,updated_at,redirection_count,number) values(
						'${res_id}','${question_type}','${question}','${options_count}','${redirection_type}','${created_at}','${updated_at}','${redirection_count}','${number}') RETURNING q_id`;
...
// =================  total_survey ===========================
//get
module.exports.total_survey = (role_id) =>{
	return new Promise((resolve,reject)=>{
		if(role_id == 3 || role_id == 2){
			const sql = `select res_id,title,desc1,start_date,final_date from research`
			...
			const Data = {
				'success':false,
				'message':'You Are not A manager or admin'
			}
			resolve(Data);
		}
	});
...
module.exports.survey_details = (id) =>{
	return new Promise((resolve,reject)=>{
		const sql = `select r.email, r.first_name, i.user_id,i.id, s.res_id,s.title, 
					 s.desc1, s.start_date, s.final_date, ss.status from invited_user as i 
					 inner join registration as r on i.user_id = r.id inner join 
					 research s on i.survey_id=s.res_id inner join survey_status
					 as ss on i.id = ss.invited_id where i.id = '${id}' `
		client.query(sql, async (err,ress)=>{
			if(err){
				const Data = {
					'success':false,
...
module.exports.submit_answer = (body) => {
	return new Promise((resolve, reject) => {
		...
		for(let ques of body.survey){
			...
			const submit_answer = `insert into answer values('${invited_id}',
								  '${user_id}','${q_id}','${answer}','${created_at}') RETURNING invited_id`
		...
		const status = `select invited_id,status from survey_status where 
		...
		client.query(status,(err,statusress)=>{
			if(statusress.rows == ''){
				...
				for(let key of statusress.rows){
					...
					if( str == "false"){
						const statusUpdate = `update survey_status set 
						updated_at = '${myDate}',status = '${true}' where
						invited_id = '${key.invited_id}' `
		...
		client.query(sql, async (err,ress)=>{
			if(err){
				...
				if(ress.rows == ''){
					const Data = {
						'success':false,
						'message':'not found any survey submited in this survey id'
					}
					resolve('not found')
				}else{
					const Data = {
